...
export default function ItemsView() {
  ...
  const [deleteBlockerMessage, setDeleteBlockerMessage] = useState(null);
  ...
  const handleBulkMoveSubmit = async (e) => {
    ...
    if (selectedItemIds.length > 0 && moveToCategoryName) {
      ...
      const itemsData = selectedItemIds.map(id => ({ id, updates }));
      await bulkUpdateItems(itemsData);
  ...
  const handleBulkDelete = async () => {
    if (selectedItemIds.length > 0) {
      if (window.confirm(`Are you sure you want to delete ${selectedItemIds.length} items? This action cannot be undone.`)) {
    ...
    <div className="flex-1 min-h-screen bg-slate-50/50 flex flex-col">
      ...
      <main className="p-8 space-y-6 flex-1 max-w-7xl w-full mx-auto">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-col gap-4 min-w-0">
            ...
            <div className="flex items-center gap-2 overflow-x-auto min-w-0 w-full pb-2 pt-1 custom-horizontal-scrollbar">
              <div className="relative shrink-0">
                ...
                <input
                  ...
                  className="w-36 bg-white border border-slate-200 rounded-lg pl-8 pr-3 py-2 text-[11px] font-semibold text-slate-700 focus:outline-none focus:ring-1 focus:ring-emerald-500 transition-all shadow-sm"
                />
              </div>

              {filteredCategoryTabs.map((cat) => (
                <button
                  key={cat}
          ...
          {/* Subcategory Tabs Row (Visible only if a specific Category is selected) */}
          {selectedCategory !== 'All' && selectedCategory !== '🔥 Trending' && (
            <div className="flex items-center pt-2">
              <div className="flex flex-nowrap overflow-x-auto gap-2 pb-2 scrollbar-hide w-full items-center">
                <span className="text-xs font-bold text-slate-400 mr-1 shrink-0">Subcategories:</span>
                <button
                ...
                {categoryDocs
                  .find(c => c.name === selectedCategory)?.subcategories?.map(sub => (
                    <button
                      key={sub}
              ...
              {selectedItemIds.length > 0 && (
                <>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 transition-colors cursor-pointer"
                  >
      ...
      {isAddCategoryModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
              ...
              <div className="max-h-60 overflow-y-auto space-y-1.5 pr-1 border border-slate-100 rounded-xl p-2 bg-slate-50/50">
                {categories.length === 0 ? (
                  ...
                  categoryDocs.map((catDoc) => {
                    const categoryItemsCount = items.filter(i => i.category === catDoc.name).length;
                    return (
                    <div
                      key={catDoc.name}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-slate-800">
                          {catDoc.name} <span className="text-slate-400 font-normal ml-1">({categoryItemsCount} items)</span>
                        </span>
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                          ...
                          <button
                            ...
                            onClick={async () => {
                              if (categoryItemsCount > 0) {
                                setDeleteBlockerMessage({
                                  title: `Cannot Delete "${catDoc.name}"`,
                                  message: `This category still contains ${categoryItemsCount} products. Please move or delete these products first.`,
                                  onAction: () => {
                                    setIsAddCategoryModalOpen(false);
                                    setSelectedCategory(catDoc.name);
                                    setSelectedSubcategory('All');
                                  }
                                });
                                return;
                      ...
                      <div className="pt-2 border-t border-slate-50 space-y-2">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {(catDoc.subcategories || []).map(sub => {
                            const subcategoryItemsCount = items.filter(i => i.category === catDoc.name && i.subcategory === sub).length;
                            return (
                            <span key={sub} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-slate-100 text-[10px] font-bold text-slate-700">
                              {sub} <span className="text-slate-400 font-normal">({subcategoryItemsCount})</span>
                              ...
                              <button type="button" onClick={async () => {
                                if (subcategoryItemsCount > 0) {
                                  setDeleteBlockerMessage({
                                    title: `Cannot Delete "${sub}"`,
                                    message: `This subcategory still contains ${subcategoryItemsCount} products. Please move or delete these products first.`,
                                    onAction: () => {
                                      setIsAddCategoryModalOpen(false);
                                      setSelectedCategory(catDoc.name);
                                      setTimeout(() => setSelectedSubcategory(sub), 0); // Need timeout because selectedCategory change resets subcategory to All
                                    }
                                  });
                                  return;
                                }
          ...
          {/* CUSTOM DELETE BLOCKER MODAL */}
          {deleteBlockerMessage && (
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
              <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setDeleteBlockerMessage(null)}></div>
              <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 p-6 max-w-sm w-full relative z-10 flex flex-col items-center text-center space-y-4">
                <div className="w-14 h-14 rounded-full bg-rose-50 flex items-center justify-center border border-rose-100">
                  <AlertTriangle className="w-7 h-7 text-rose-500" />
                </div>
                <div>
                  <h3 className="text-lg font-black text-slate-800">{deleteBlockerMessage.title}</h3>
                  <p className="text-sm text-slate-500 mt-2 leading-relaxed">{deleteBlockerMessage.message}</p>
                </div>
                <button
                  type="button"
                  onClick={() => {
                    if (deleteBlockerMessage.onAction) {
                      deleteBlockerMessage.onAction();
                    }
                    setDeleteBlockerMessage(null);
                  }}
                  className="w-full py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-sm font-bold transition-all mt-2"
                >
                  View these products
                </button>
              </div>
