...
const POLICY_DOC_REF = doc(db, 'settings', 'disclaimer');
...
export default function disclaimerView() {
  ...
  useEffect(() => {
    async function loadPolicy() {
      try {
        ...
        setError('Failed to load Disclaimer content.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const joditConfig = useMemo(() => ({
    readonly: false,
    placeholder: 'Start writing your Disclaimer here...',
    height: 600,
    style: {
      fontFamily: 'inherit',
      color: '#334155'
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-semibold text-slate-500">Loading Disclaimer…</p>
        </div>
      </div>
    );
  }
      ...
      <div className="sticky top-0 z-20 bg-white/80 backdrop-blur-xl border-b border-slate-200">
        <div className="px-6 lg:px-10 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-extrabold text-slate-800 tracking-tight">Disclaimer</h1>
